...
<CodeBlock language="bash">{`docker run -d --name bifromq -p 1883:1883 bifromq/bifromq:latest`}</CodeBlock>

Set memory limits:

<CodeBlock language="bash">{`docker run -d -m 10G -e MEM_LIMIT='10737418240' --name bifromq -p 1883:1883 bifromq/bifromq:latest`}</CodeBlock>

Pass custom JVM options with `JVM_HEAP_OPTS` as needed.

## Plugin packages (Maven Central)

Java plugins are published under `org.apache.bifromq`. Match the plugin version to your BifroMQ deployment:
