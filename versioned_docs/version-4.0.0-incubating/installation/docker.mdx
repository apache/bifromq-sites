...
<CodeBlock language="bash">{`docker run -d --name bifromq -p 1883:1883 apache/bifromq:${useDocsVersion().label}`}</CodeBlock>
## Memory Constraints
...
<CodeBlock language="bash">{`docker run -d -m 10G -e MEM_LIMIT='10737418240'--name bifromq -p 1883:1883 apache/bifromq:${useDocsVersion().label}`}</CodeBlock>
...
<CodeBlock language="bash">{`docker run -d -m 10G -e JVM_HEAP_OPTS='-Xms2G -Xmx4G -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=500m -XX:MaxDirectMemorySize=1G' --name bifromq -p 1883:1883 apache/bifromq:${useDocsVersion().label}`}</CodeBlock>
...
<CodeBlock language="yaml">{`services:
  bifromq-node1:
    image: apache/bifromq:${useDocsVersion().label}
